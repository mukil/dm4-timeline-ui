    ...
    return {
        ...
        contents: [
            ...
            if (element.$.localName == "span" || element.$.localName != "div") {
                ...
                if (mathjaxDiv.getAttribute('class') === 'MathJax_Display') formId = mathjaxDiv.$.firstChild.id
                // FIXME check if we have the output container generated by mathjax at hand
                if (mathjaxDiv) {
                    newElement = mathjaxDiv.getAscendant('div')
                    if (newElement.getAttribute('class') === 'MathJax_Display') formId = newElement.$.firstChild.id
            ...
            if (!this.insertMode) {
                ...
                _this.editingFormulaId = formId.replace('-Frame', '')
                // MathJax
            ...
            var selection = editor.getSelection()
            var element = selection.getStartElement()
            var newElement = element.getAscendant('div')
            if (newElement.getAttribute('class') !== 'math-output') {
                newElement = newElement.getAscendant('div')
                if (newElement.getAttribute('class') !== 'math-output') {
                    newElement = newElement.getAscendant('div')
            ...
            if (dialog.insertMode) {
                ...
                if (newElement.getAttribute('class') === 'math-output') {
                    ...
                    preview.setAttribute('class', 'math-preview')
                    preview.setText(DELIMITER + math + DELIMITER)
                    // remove old formula element
                    var oldMathJaxElement = getMathJaxElementById(_this.editingFormulaId)
                    oldMathJaxElement.Remove()
                    // and append updated preview object
                    newElement.append(preview)
                } else {
